...
for (( i=1; i<=${COUNT}; i++ ))
do
    gcloud compute instances create ${SERVER}-${i} --image=$IMAGE --image-project=centos-cloud \
     --boot-disk-type pd-standard --boot-disk-size 200 --zone ${REGION}-${ZONES[$i-1]} \
     --machine-type=${MACHINE_TYPE} \
     --can-ip-forward --tags ${SERVER},${SERVER}-${i} \
     --disk name=${DISK}1-${i} \
     --scopes compute-rw,logging-write,monitoring-write,sql,storage-full
     ...
     echo "Creating /etc/sysconfig/network-scripts/ifcfg-eth0:1 file on  ${SERVER}-${i} ..."
     gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command \
     'echo -e "# static IP
DEVICE=eth0:1
IPADDR='${STATIC_IP[$i-1]}'
NETMASK=255.255.255.0
ONBOOT=yes
BOOTPROTO=static" | sudo tee /etc/sysconfig/network-scripts/ifcfg-eth0:1'
     echo " "
     echo "Enabling eth0:1 network interface on ${SERVER}-${i} ..."
     gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command "sudo ifup eth0:1"
     echo " "
     #
...
#     echo " "
     ...
     gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command "yes | sudo yum install -y centos-release-gluster && sudo yum install -y glusterfs gluster-cli glusterfs-libs glusterfs-server lvm2"
     gcloud compute ssh --zone ${REGION}-${ZONES[$i-1]} ${SERVER}-${i} --command "yes | sudo systemctl enable glusterd.service && sudo systemctl start glusterd.service"
     echo " "
